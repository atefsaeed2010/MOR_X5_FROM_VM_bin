...
class RinggroupsController < ApplicationController
  ...
  def index
    ...
    # page params
    @ringgroups_size = current_user.ringgroups.where(arr[:conditions]).all.size.to_i
    ...
    @ringgroups = current_user.ringgroups.where(arr[:conditions]).all
  ...
  def create
    ...
    if params[:dialplan][:data2] && extension_exists?(ext)
      @page_title = _('Ring_group_new')
      @page_icon = "add.png"
  ...
  def update
    ...
    @dialplan = @ringgroup.dialplan
    if params[:dialplan][:data2] && extension_exists?(params[:dialplan][:data2],  @dialplan.try(:data2))
      @ringgroup.errors.add(:extension_error, _('Extension_is_used'))
    end

    if @ringgroup.errors.size.zero? && @ringgroup.update_attributes(params[:ringgroup].reject{|k,v| k == 'user_id'})
  ...
  def assign_device
    ...
    r_old = RinggroupsDevice.where({:ringgroup_id => @ringgroup.id}).order('priority DESC').first
  ...
  def device_sort
    params[:sortable_list].each_index do |i|
      item = RinggroupsDevice.where({:device_id => params[:sortable_list][i], :ringgroup_id => @ringgroup.id}).first
