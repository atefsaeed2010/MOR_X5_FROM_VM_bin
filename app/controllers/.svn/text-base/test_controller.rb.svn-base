...
#  include Google4R::Checkout
  ...
  def time_zones
    render text: "<table><tr><th>NAME</th><th>VALUE</th></tr>#{ActiveSupport::TimeZone.all.each_with_index.collect { |tz, index| "<tr #{"style='background-color:#EEE'" if index % 2 == 1}><td id='tz_name_#{index}'>#{tz.to_s}</td><td id='tz_value_#{index}'>#{tz.name.to_s}</td></tr>" }.join()}</table>".html_safe
  end
  ...
  def make_select
    ...
    if params[:id] and @table
      if @table.to_s != 'sessions'
        @select = @table.singularize.titleize.gsub(' ', '').constantize.where({:id => params[:id]}).first
      else
