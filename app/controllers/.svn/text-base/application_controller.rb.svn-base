...
class ApplicationController < ActionController::Base
  ...
  if !Rails.env.development?
    rescue_from Exception do |exc|
      ...
      logger.fatal exc.backtrace.collect { |except| except.to_s }.join("\n")
  ...
  helper_method :convert_curr, :allow_manage_providers?, :allow_manage_dids?, :allow_manage_providers_tariffs?,
                ...
                :nice_date_time, :reseller_active?, :rec_active?, :cc_active?, :ad_active?, :change_exchange_rate,
                :ccl_active?, :is_number?, :is_numeric?, :load_ok?, :pbx_active?, :send_email_dry,
                :user_tz, :strip_params, :x5_functionality, :retry_lock_error
  ...
  helper_method :callback_active?, :call_shop_active?, :payment_gateway_active?,
                ...
                :skp_active?, :cc_single_login_active?, :erp_active?, :admin?, :reseller?, :user?, :accountant?, :reseller_pro_active?,
                :provider_billing_active?, :providers_enabled_for_reseller?
  ...
  def nice_device(device)
    d = ''
    if device
      d = device.device_type.to_s + "/"
      d += device.name.to_s if device.device_type != "FAX"
      d += device.extension.to_s if device.device_type == "FAX" or device.name.length == 0
    end
  ...
  def free_extension
    ran_min, ran_max = Confline.get_device_range_values

    sql = "SELECT c AS free, COUNT(v.c) AS x FROM (
              (SELECT #{ran_min} AS c)
              UNION ALL
              (SELECT data2 c FROM dialplans  WHERE (dptype = 'queue' AND data2 REGEXP '^[0-9]+$' = 1 AND data2 BETWEEN #{ran_min} AND #{ran_max}) GROUP BY c)
              UNION ALL
    ...
    used_extensions << Extline.select(:exten).all.map{|extline| extline.exten.to_i}
    used_extensions << Device.select(:extension).all.map{|device| device.extension.to_i}
    used_extensions << AstQueue.select(:extension).all.map{|queue| queue.extension.to_i}
    used_extensions = used_extensions.flatten
  ...
  def renew_session(user)
    ...
    session[:usertype_id] = Role.where(name: session[:usertype].to_s).first.id.to_i
    ...
    user_is_reseller = user.usertype == "reseller"
    ...
    userid = user_is_reseller ? user.id : user.owner_id
    ...
    session[:items_per_page] = 1 if session[:items_per_page].to_i < 1
    format = Confline.get_value("Date_format", user_is_reseller ? user.id : user.owner_id).to_s
    ...
    if reseller? and Confline.get_value("Paypal_Disable_For_Reseller").to_i == 1
  ...
  def set_erp_settins(user)
    ...
    session[:erp_domain], session[:erp_login], session[:erp_password] = Confline.get_erp_values(cor_id)
  end
  ...
  def flags_to_session(force_owner = nil)
    unless force_owner and force_owner.class == User
      if current_user
        ...
        tra = UserTranslation.includes(:translation).where(:active => 1, :user_id => 0).references(:translation).order("user_translations.position ASC")
...
=end
  ...
  def my_rescue_action_in_public(exception)
    ...
    begin
      ...
      if important_exception(exception)
        ...
        if exception_class.include?("Mysql2::Error") and exception.message.include?("Unknown column")
          flash_notice = _('Database_Error')
          flash_help_link = ''
          Action.new(:user_id => session[:user_id].to_i, :date => Time.now.to_s(:db), :action => "error", :data => 'Database_Error', :data2 => exception.message).save
        end
 ...
 # Returns correct owner_id if usertype is accountant
  ...
  def check_csv_file_seperators(file, min_collum_size = 1, return_type = 1, opts = {})
    file = file.to_s.encode('UTF-8', 'binary', invalid: :replace, undef: :replace, replace: '').split("\n")
    not_words = ""
    objc = []
  ...
  def allow_manage_providers?
    admin? or current_user.reseller_allow_providers_tariff?
  end
  ...
  def access_denied_not_x5_functionality18
    access_denied
  end

  def ApplicationController::send_email_dry(from = "", to = "", message = "", subject = "", files = "", smtp = "'localhost'", message_type = 'plain')
    cmd = "/usr/local/mor/sendEmail -s #{smtp} -f '#{from}' -t '#{to}' -u '#{subject}' -o 'message-content-type=text/#{message_type}'"
  ...
  def strip_params
    params.each do |n|
      if n[1].respond_to?('strip')
        n[1].strip!
      else
        # if param is a hash of other params
        n[1].each do |m|
          m[1].strip! if m[1].respond_to?('strip')
        end
      end
    end
  end

  def x5_functionality(number)
    functionality = "x5_functionality_#{number}".to_sym
    confline = lambda { Confline.get_value(functionality).to_i == 1 }
    functionality_active(functionality, confline)
  end

  def x6_functionality(number)
    functionality = "x6_functionality_#{number}".to_sym
    confline = lambda { Confline.get_value(functionality).to_i == 1 }
    functionality_active(functionality, confline)
  end
  ...
  def accountant?
    session[:usertype].to_s == "accountant" if session[:usertype].present?
  end
