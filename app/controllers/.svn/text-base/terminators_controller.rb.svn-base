...
=end
  ...
  def providers
    ...
    if current_user.usertype == 'reseller'
      @assigned = Provider.where(["providers.terminator_id = ? AND (providers.user_id = ? OR (providers.common_use = 1 AND id IN (SELECT provider_id FROM common_use_providers where reseller_id = #{current_user_id})))", terminator_id, current_user_id]).order("name ASC")
      @not_assigned = Provider.where(["providers.terminator_id = 0 AND (providers.user_id = ? OR (providers.common_use = 1 AND id IN (SELECT provider_id FROM common_use_providers where reseller_id = #{current_user_id})))", current_user_id]).order("name ASC")
    else
      @assigned = Provider.where(["providers.terminator_id = ? AND (providers.user_id = ? OR providers.common_use = 1)", terminator_id, current_user_id]).order("name ASC")
      @not_assigned = Provider.where(["providers.terminator_id = 0 AND (providers.user_id = ? OR providers.common_use = 1)", current_user_id]).order("name ASC")
    end
  end
...
=end
  ...
  def provider_add
    prov = Provider.where(["providers.id = ? AND (providers.user_id = ? or providers.common_use = 1)", params[:provider_id], session[:user_id]]).first
    unless prov
      flash[:notice] = _('Provider_was_not_found')
      redirect_to :action => :terminators and return false
    end
    prov.terminator = @terminator
    if prov.save
