...
class CommonUseProvidersController < ApplicationController
  ...
  def index
    ...
    @resellers = User.select("users.*, #{SqlExport.nice_user_sql}").where('usertype = "reseller" AND own_providers = 1')
  ...
  def update
    ...
    if check_record(params[:select_reseller], params[:select_provider], params[:id].to_i)
      ...
      #update record
      data = @commmon_use_provider
      data.update_attributes(reseller_id: params[:select_reseller], provider_id: params[:select_provider], tariff_id: params[:select_tariff])
  ...
  def let_resellers_use_all_common_use_providers
    if admin?
      providers = Provider.where('common_use = 1 and user_id = 0').order('name asc')
      resellers = User.where('usertype = "reseller" AND own_providers = 1')
      #create record
      if resellers
  ...
  def check_record(reseller, provider, id = nil)
    ...
    if CommonUseProvider.where(["reseller_id = ? AND provider_id = ? "+cond, reseller, provider]).first
      return true
    end
  ...
  def find_data
    @commmon_use_provider = CommonUseProvider.where({:id => params[:id]}).first
