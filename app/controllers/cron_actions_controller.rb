...
class CronActionsController < ApplicationController
  ...
  def index
    ...
    @page_icon = "clock.png"
    @help_link = 'http://wiki.kolmisoft.com/index.php/Cron_actions'
  ...
  def find_data_for_cron
    ...
    @providers = Provider.where(user_id: current_user.id).order('name ASC').all
    @provider_tariffs = Tariff.where(purpose: 'provider', owner_id: current_user.id).order('name ASC').all
    @lcrs = Lcr.where("user_id = ? ", current_user.id).order('name ASC').all
