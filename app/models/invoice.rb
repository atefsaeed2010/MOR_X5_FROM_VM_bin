...
class Invoice < ActiveRecord::Base
  ...
  def filename(type, file_type, prefix = nil, dc = nil)
    ...
    long_name = "#{file_prefix}-#{self.user.first_name}_#{self.user.last_name}-#{self.user_id}-#{self.number}-#{self.issue_date}#{dc_string}"
...
=end
  ...
  def generate_invoice_detailed_pdf(current_user, dc, ex, nc, cde, gde, show_avg_rate, testing_mode = false)
    ...
    invoicedetails.map do |item|
      if (item.invdet_type > 0 or item.name == _("Did_owner_cost") or item.name == _("SMS")) and item.name != 'Calls'
        ii = []
    ...
    if options[:show_avg_rate] == 0
      pdf.table(items,
                ...
                :align_headers => {0 => :right, 1 => :right, 2 => :right, 3 => :right},
  ...
  def generate_invoice_by_cid_pdf(current_user, dc, ex, nc, cde, gde, testing_mode = false)
    ...
    limit = Confline.get_value("#{type}Invoice_page_limit", user.owner).to_i ? 20 : Confline.get_value("#{type}Invoice_page_limit", user.owner).to_i
    max_limit = Confline.get_value("Max_PDF_pages").to_i
    limit = max_limit if limit > max_limit and max_limit > 0
    ...
    sql = "SELECT calls.src,
                  #{options[:did_sql_price]},
                  #{options[:did_inc_sql_price]},
           ...
           WHERE devices.user_id = #{user.id}
                 AND calls.calldate
                 BETWEEN '#{period_start} 00:00:00' AND '#{period_end} 23:59:59'
                 AND calls.disposition = 'ANSWERED'
                 AND billsec > 0
                 AND card_id = 0 #{options[:zero_calls_sql]}
           GROUP BY calls.src;"
    ...
    if invoicedetails and invoicedetails.size.to_i > 0
      invoicedetails.each do |id|
        if (id.invdet_type > 0 or id.name == _("Did_owner_cost") or id.name == _("SMS")) and id.name != 'Calls'
          if id.invdet_type > 0
  ...
  def pay(params, owner_id)
    if self.paid == 0
      ...
      self.paid_date = Time.now.strftime("%Y-%m-#{params[:date][:day]}")
