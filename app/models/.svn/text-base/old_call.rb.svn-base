...
class OldCall < ActiveRecord::Base
  ...
  def OldCall.last_calls(options={})
    ...
    if options[:current_user].usertype == 'reseller'
      ...
      select << "(#{SqlExport.calls_old_reseller_profit_sql} * #{options[:exchange_rate]} ) AS profit"
    else
      if options[:current_user].usertype == 'user'
        ...
        select << "(#{SqlExport.calls_old_admin_profit_sql} + #{options[:s_user_id] == '-2' ? 'calls_old.did_price + calls_old.did_prov_price' : '(IF(calls_old.user_id = ' + options[:s_user_id].to_s + ', calls_old.did_prov_price, calls_old.did_price - calls_old.did_inc_price))'}) * #{options[:exchange_rate]} AS profit"
      end
    end
    select << "IF(resellers.id > 0, #{SqlExport.nice_user_sql("resellers", nil)}, '') AS 'nice_reseller'"
    OldCall.find(:all, :select => select.join(", \n"), :conditions => [cond.join(" \nAND "), *var], :joins => jn.join(" \n"), :order => options[:order], :limit => "#{((options[:page].to_i - 1) * options[:items_per_page].to_i).to_i}, #{options[:items_per_page].to_i}")
  end
  ...
  def OldCall.last_calls_total_stats(options={})
    ...
    if options[:current_user].usertype == 'reseller'
      ...
      profit = "(SUM(IF(calls_old.user_id = #{options[:current_user].id}, 0, #{SqlExport.calls_old_reseller_profit_sql})) * #{options[:exchange_rate].to_d}) AS total_profit"
      ...
      profit = "(SUM(#{SqlExport.calls_old_admin_profit_sql} + #{options[:s_user_id] == '-2' ? 'calls_old.did_price + calls_old.did_prov_price' : '(IF(calls_old.user_id = ' + options[:s_user_id].to_s + ', calls_old.did_prov_price , calls_old.did_price - calls_old.did_inc_price))'}) * #{options[:exchange_rate].to_d}) AS total_profit"
      user_price = SqlExport.calls_old_user_price_no_dids_sql
    ...
    OldCall.find(
        ...
        :select => "
                 ...
                 SUM(IF(calls_old.user_id = #{options[:current_user].id}, #{SqlExport.calls_old_user_price_sql}, #{options[:current_user].usertype == "user" ? SqlExport.calls_old_user_did_price_sql : SqlExport.calls_old_user_price_sql})) * #{options[:exchange_rate].to_d} as total_user_price_with_dids,
  ...
  def OldCall.last_calls_csv(options={})
    ...
    if options[:current_user].show_billing_info == 1 and options[:can_see_finances]
      if options[:current_user].usertype == 'reseller'
        ...
        s << SqlExport.replace_dec("(IF(#{SqlExport.calls_old_user_price_sql} != 0 , (#{SqlExport.calls_old_user_price_sql}), 0) * #{options[:exchange_rate]} ) ", options[:column_dem], 'user_price')
        headers << "'#{_('DID_Number')}'" + ' AS did'
        ...
        s << SqlExport.replace_dec("(IF(calls_old.did_price IS NULL OR users.owner_id != #{options[:current_user].id}, calls_old.did_price, 0) * #{options[:exchange_rate]} ) ", options[:column_dem], 'did_price')
    ...
    if options[:current_user].usertype == "admin" or options[:current_user].usertype == "accountant"
      if options[:can_see_finances]
        ...
        if options[:s_user_id] != '-2'
          s << SqlExport.replace_dec("(IF(calls_old.user_id = #{options[:user].id},#{SqlExport.calls_old_admin_profit_sql} + calls_old.did_prov_price,calls_old.did_price) * #{options[:exchange_rate]})", options[:column_dem], 'profit')
        else
      ...
      if options[:can_see_finances]
        headers << "'#{_('Profit')}'" + ' AS profit'
        s << SqlExport.replace_dec("(#{SqlExport.calls_old_reseller_profit_sql} * #{options[:exchange_rate]})", options[:column_dem], 'profit')
      end
    end
    ...
    unless options[:s_country] and !options[:s_country].blank?
      sql += "LEFT JOIN destinations ON destinations.prefix = IFNULL(calls_old.prefix, '') "
    end
    sql += jn.join(' ')
...
#      MorLog.my_debug(mysql_res.inspect.to_s)
  ...
  def OldCall.calls_for_laod_stats(options={})
    ...
    c2 = OldCall.find(:all, :conditions => [cond.join(' AND ').to_s] + var, :select => 'calldate, duration')
    cond << "disposition = 'ANSWERED'"
    calls1 = OldCall.find(:all, :conditions => [cond.join(' AND ').to_s] + var, :select => 'calldate, duration')
    return calls1, c2
  end
  ...
  def OldCall.country_stats(options={})
    ...
    calls = OldCall.find(:all,
                      :conditions => [cond.join(' AND ').to_s] + var,
                      :select => "destinations.direction_code as 'direction_code', destinationgroups.id, destinationgroups.flag as 'dg_flag', destinationgroups.name as 'dg_name', destinationgroups.desttype as 'dg_type',  COUNT(*) as 'calls', SUM(IF(calls_old.billsec > 0, calls_old.billsec, CEIL(calls_old.real_billsec) )) as 'billsec', SUM(#{provider_price}) as 'selfcost', SUM(#{user_price}) as 'price'",
                      :joins => "LEFT JOIN destinations ON (destinations.prefix = calls_old.prefix) JOIN destinationgroups ON (destinations.destinationgroup_id = destinationgroups.id) #{SqlExport.calls_old_left_join_reseler_providers_to_calls_sql}",
                      :group => 'destinationgroups.id',
                      :order => 'destinationgroups.name ASC, destinationgroups.desttype ASC')

    calls_for_pie_graph = OldCall.find(:all,
                                    :conditions => [cond.join(' AND ').to_s] + var,
                                    :select => "destinationgroups.name as 'dg_name', destinationgroups.desttype as 'dg_type',SUM(IF(calls_old.billsec > 0, calls_old.billsec, CEIL(calls_old.real_billsec) )) as 'billsec'",
                                    :joins => "LEFT JOIN destinations ON (destinations.prefix = calls_old.prefix) JOIN destinationgroups ON (destinations.destinationgroup_id = destinationgroups.id) #{SqlExport.calls_old_left_join_reseler_providers_to_calls_sql}",
                                    :group => 'destinationgroups.id',
                                    :order => 'billsec desc')

    calls_for_price = OldCall.find(:all,
                                :conditions => [cond.join(' AND ').to_s] + var,
    ...
    calls_for_profit = OldCall.find(:all,
                                 :conditions => [cond.join(' AND ').to_s] + var,
  ...
  def OldCall.cardgroup_aggregate(options={})
    ...
    if options[:csv].to_i == 0
      s << "IF(calls_old.prefix = '', '#{_('Calls_To_Dids')}', calls_old.prefix ) AS prefix, directions.name as 'dir_name', destinations.direction_code AS 'code', destinations.subcode AS 'subcode', destinations.name AS 'dest_name'"
    else
      if options[:destination_grouping].to_i == 1
        s << SqlExport.column_escape_null("CONCAT(directions.name, ' ', destinations.subcode, ' ', destinations.name, ' (',  calls_old.prefix, ') ' )", "direct_name", "#{_('Calls_To_Dids')}")
      else
        s << SqlExport.column_escape_null("CONCAT(directions.name, ' ', destinations.subcode, ' (',  calls_old.prefix, ') ')", "direct_name", "#{_('Calls_To_Dids')}")
      end
    end
    s << "cardgroups.name AS  'cardgroup_name'"
    s << SqlExport.column_escape_null("SUM(IF(calls_old.disposition = 'ANSWERED', calls_old.billsec, 0))", 'duration', 0)
    s << SqlExport.column_escape_null("SUM(IF(calls_old.disposition = 'ANSWERED', 1,0))", 'answered_calls', 0)
    s << SqlExport.column_escape_null("COUNT(*)", 'total_calls', 0)
    s << SqlExport.column_escape_null("SUM(IF(calls_old.disposition = 'ANSWERED', 1,0))/COUNT(*)*100", 'asr', 0)
    s << SqlExport.column_escape_null("SUM(IF(calls_old.disposition = 'ANSWERED', calls_old.billsec, 0))/SUM(IF(calls_old.disposition = 'ANSWERED', 1,0))", 'acd', 0)
    s << SqlExport.column_escape_null("SUM(calls_old.provider_price)", "provider_price", 0)
    s << SqlExport.column_escape_null("SUM(calls_old.user_price)", "user_price", 0)
    s << SqlExport.column_escape_null("SUM(calls_old.user_price) - SUM(calls_old.provider_price)", 'profit', 0)
    s << SqlExport.column_escape_null("IF((SUM(calls_old.user_price) != 0 OR SUM(calls_old.provider_price) != 0),(((SUM(calls_old.user_price) - SUM(calls_old.provider_price)) / SUM(calls_old.user_price)) * 100),0)", 'margin', 0)
    s << SqlExport.column_escape_null("IF((SUM(calls_old.user_price) != 0 OR SUM(calls_old.provider_price) != 0),(((SUM(calls_old.user_price) / SUM(calls_old.provider_price)) * 100 ) - 100), 0)", 'markup', 0)
  ...
  def OldCall.analize_cdr_import(name, options)
    ...
    arr[:calls_in_db] = OldCall.count(:all, :conditions => {:reseller_id => current_user}).to_i
...
=end
  ...
  def OldCall.last_calls_parse_params(options={})
    ...
    if options[:current_user].usertype == "reseller" and !options[:user]
      cond << "(calls_old.reseller_id = ? OR calls_old.user_id = ? OR users.owner_id = ?)"
      var += [options[:current_user].id, options[:current_user].id, options[:current_user].id]
    end
