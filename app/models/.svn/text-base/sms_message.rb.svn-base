...
class SmsMessage < ActiveRecord::Base
  ...
  attr_accessible :prefix, :number, :clickatell_message_id
  ...
  def sms_set_rates_and_user(user, sms_numbers, user_tariff, number, api = 0)
    ...
    if user.owner_id != 0
      ...
      r_price = (reseller_rate.price.to_d / Currency.where(["name='#{reseller_tariff.currency}'"]).first.exchange_rate.to_d).to_d
    ...
    price = (user_rate.price.to_d / Currency.where(["name='#{user_tariff.currency}'"]).first.exchange_rate.to_d).to_d
