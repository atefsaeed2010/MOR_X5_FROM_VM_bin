...
class Lcr < ActiveRecord::Base
  ...
  def lcr_partials_destinations(options = {})
    search = []
    search << " AND destinations.name LIKE '#{options[:s_destination]}'" if options[:s_destination].present?
    search << " AND destinations.prefix LIKE '#{options[:s_prefix]}'" if options[:s_prefix].present?
    search << " AND destinations.direction_code = '#{options[:s_country]}'" if options[:s_country].present?
